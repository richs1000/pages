...
QuestionBankModel.prototype.checkAnswer = function (studentAnswer) {
    ...
    if(studentAnswer.length != this.answers[0].length)
    	return false;
    else
	    for (var i = 0; i < this.answers[0].length; i++) {
			if(studentAnswer[i] == this.answers[0][i])
				continue;
			else
				return false;
	    }
...
QuestionBankModel.prototype.createNewQuestions = function(_datatype) {
	...
	list = []
	list.push(_datatype.createquestion())
	
    this.questions = [["Given these type synonyms which one of the types below are interchangeable with each other?</br></br> type date = int * int * int </br> type s = string",
    					"",  list]];
...
QuestionBankModel.prototype.chooseQuestion = function(_firstQuestion, _lastQuestion) {
    ...
    for (index = 0; index < questionTemplate.length - 2; index++) {
...
QuestionBankModel.prototype.setAnswers = function(_datatype) {
	...
	this.answers.push(_datatype.getAnswer(this))
}
